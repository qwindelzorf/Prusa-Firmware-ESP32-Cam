...
all: certs esp32-cam esp32-wrover-dev esp32-s3-eye-22 xiao-esp32-s3 esp32-s3-cam esp32-s3-wroom-freenove
...
# Download/update X.509 certificates used by the firmware
.PHONY: certs
certs:
	@echo "Updating X.509 certificates..."
	@cd $(SRCDIR) && \
		TMPDIR=$$(mktemp -d) && \
		echo "Fetching Prusa certificates..." && \
		echo -n | openssl s_client -servername connect.prusa3d.com -connect connect.prusa3d.com:443 2>/dev/null | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > $$TMPDIR/connect_prusa3d_com.crt && \
		echo "Writing Certificate.h..." && \
		{ printf '/*\n   @file Certificate.h\n\n   @brief X.509 SSL certificate for communication with Prusa servers\n\n   Auto-generated by make certs.\n*/\n\n#pragma once\n\n' ; printf 'const char root_CAs[] PROGMEM = R"rawliteral(\n' ; sed -n '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' $$TMPDIR/connect_prusa3d_com.crt ; printf '\n)rawliteral";\n\n/* EOF */\n' ; } > Certificate.h && \
		echo "Fetching GitHub certificates..." && \
		for host in github.com objects.githubusercontent.com raw.githubusercontent.com api.github.com; do \
			echo -n | openssl s_client -servername $$host -connect $$host:443 2>/dev/null | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > $$TMPDIR/$$host.crt; \
		done && \
		echo "Writing Certificate_ota.h..." && \
		{ printf '/*\n   @file Certificate_ota.h\n\n   @brief X.509 SSL certificate for communication with GitHub servers for OTA update\n\n   Auto-generated by make certs.\n*/\n\n#pragma once\n\n' ; printf 'const char root_CAs_ota[] PROGMEM =  R"rawliteral(\n' ; for f in $$TMPDIR/*.crt; do sed -n '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' $$f ; printf '\n' ; done ; printf '\n)rawliteral";\n\n/* EOF */\n' ; } > Certificate_ota.h && \
		rm -rf $$TMPDIR && \
		echo "Certificates updated in $(SRCDIR)/"
...
$1:
	...
	@echo "Build for $$(BOARD_DISPLAY_$1) completed. Output files are in $(OUT_DIR)/$1.zip and $(OUT_DIR)/$$(BOARD_OUT_$1)"
endef
